...
export default function Home() {
	...
	useEffect(() => {
		...
		Object.keys(filters).forEach((key) => {
			if (filters[key].length > 0) {
				if (key === "bald" && filters[key][0]) {
					result = result.filter(
						(item) =>
							!item.meta.attributes.some((attr) => attr.trait_type === "hair")
					);
				} else if (key === "naked" && filters[key][0]) {
					result = result.filter(
						(item) =>
							!item.meta.attributes.some((attr) => attr.trait_type === "armor")
					);
				} else if (key === "fullNaked" && filters[key][0]) {
					result = result.filter(
						(item) =>
							!item.meta.attributes.some(
								(attr) => attr.trait_type === "armor"
							) &&
							!item.meta.attributes.some(
								(attr) => attr.trait_type === "mask"
							) &&
							!item.meta.attributes.some((attr) => attr.trait_type === "under")
					);
				} else if (key === "noUnknown" && filters[key][0]) {
					result = result.filter(
						(item) =>
							!item.meta.attributes.some(
								(attr) => attr.trait_type === "lost pilgrim"
							)
					);
				} else if (key === "noGreatOnes" && filters[key][0]) {
					result = result.filter(
						(item) =>
							!item.meta.attributes.some(
								(attr) => attr.trait_type === "great ones"
							)
					);
				} else {
					result = result.filter((item) =>
						item.meta.attributes.some(
							(attr) =>
								attr.trait_type === key && filters[key].includes(attr.value)
						)
					);
				}
			}
	...
	const additionalFilters = [
		{ name: "Bald", key: "bald" },
		{ name: "Naked", key: "naked" },
		{ name: "Full Naked", key: "fullNaked" },
		{ name: "No Unknown", key: "noUnknown" },
		{ name: "No Great Ones", key: "noGreatOnes" },
	];
			...
			<aside className="w-64 bg-gray-800 p-4 shadow-lg overflow-y-auto">
								...
								{uniqueAttributes(type).map((value) => (
									<div key={value} className="mb-1">
										<input
											...
											name={type}
											value={value}
											checked={filters[type]?.includes(value)} // Ensure checkbox reflects current filter state
				...
				<div className="mt-4">
					...
					<select
						...
						className="ml-2 p-1 border rounded bg-gray-700 text-gray-200"
					>
